...
def _run_legacy_api_fallback(job):
    ...
    if technical_failures:
        for retry_index,item in enumerate(technical_failures,1):
            student=item["student"]; status,technical,error_text=check_one(student); save_result(student,status,total+retry_index)
    remaining_tech=[]
...
for key,default in [("logged_in",False),("is_admin",False),("office",None),("update_locked",False),("update_start_requested",False),("update_preparing",False),("pending_file_bytes",None),("pending_filename","") ,("active_job_id",None)]:
        ...
        with tab1:
            ...
            if st.button("تسجيل الدخول",key="login_btn"):
                if username==ADMIN_USERNAME and password==ADMIN_PASSWORD:
                    ...
                    if ok: st.session_state.logged_in=True; st.session_state.office=result; st.session_state.update_locked=False; st.session_state.update_start_requested=False; st.session_state.update_preparing=False; st.rerun()
...
def _prepare_update_job(office_id, source_type, source_name, file_bytes, source_url, file_name):
    try:
        ...
        job=create_job(office_id,src,file_name or source_name)
        log_activity(office_id,"إنشاء مهمة تحديث حالات",file_name or source_name,{"job_id":job["id"],"students":count},data_source_id=src["id"])
        return job,count
    except Exception as exc:
...
def render_processing():
    ...
    if st.session_state.update_start_requested and not st.session_state.active_job_id:
        st.info("⏳ سيبدأ التحديث خلال ثواني…")
        if not st.session_state.update_preparing:
            ...
            def prepare_and_start():
                job,count=_prepare_update_job(office_id,source_type,source_name,file_bytes,saved_link if is_gsheet_source else None,filename or source_name)
                if job:
                    ...
                    st.session_state.update_thread=threading.Thread(target=_background_update_job,args=(job["id"],),daemon=True)
    ...
    if job:
        ...
        if status in {"processing","done","failed"}:
            st.info("▶️ بدأ التحديث. جاري فحص الطلاب وتحديث الحالات…") if status=="processing" else None
        elif status=="pending":
            st.info("⏳ سيبدأ التحديث خلال ثواني…")
        ...
        if rows:
            latest={}
            for r in rows:
                key=(str(r.get("student_index") or ""),str(r.get("student_name") or "").strip().lower(),str(r.get("status") or "").strip())
                if key[0] and key[1]: latest[key]=r
            shown=sorted(latest.values(),key=lambda r:int(r.get("student_index") or 0))
            total=max([int(r.get("total") or 0) for r in rows] or [0])
            completed=len({int(r.get("student_index") or 0) for r in shown if int(r.get("student_index") or 0)<=total})
            st.progress(min(completed/max(total,1),1.0))
            st.caption(f"طالب {min(completed,total) if total else completed} من {total}")
            last=shown[-1]
            st.info(f"🔄 آخر طالب تم فحصه: **{last.get('student_name') or 'طالب'}** — الحالة الجديدة: **{last.get('status') or ''}**")
            display_rows=[{"#":i+1,"اسم الطالب":r.get("student_name", ""),"الحالة":r.get("status", "")} for i,r in enumerate(shown)]
        ...
        elif status=="failed": st.error("حصلت مشكلة أثناء التحديث. جرّبي تسجيل الدخول مرة تانية لو المشكلة استمرت.")
    elif st.session_state.update_locked and not st.session_state.update_start_requested:
