...
def render_processing():
    ...
    if job:
        ...
        if status in {"processing","done","failed"} and st.session_state.update_notice=="waiting": st.session_state.update_notice="started"
        if status=="pending":
            if st.session_state.update_notice=="waiting": st.info("⏳ سيبدأ التحديث خلال ثواني…")
            else: st.info("⏳ جاري تجهيز التحديث…")
