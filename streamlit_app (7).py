...
def get_students(office_id,search=""):
    ...
    rows=[item[1] for item in latest.values()]; rows=sorted(rows,key=lambda r:str(r.get("student_name") or "").lower())
    if search.strip():
...
def _run_legacy_api_fallback(job):
    ...
    def check_one(student):
        ...
        except Exception as exc: technical=True; error_text=str(exc); status="خطأ فني في الفحص"
...
for key,default in [("logged_in",False),("is_admin",False),("office",None),("update_locked",False),("pending_file_bytes",None),("pending_filename","") ,("active_job_id",None),("update_notice","idle")]:
...
if not st.session_state.logged_in and not st.session_state.is_admin:
    ...
    with right:
        ...
        with tab1:
            ...
            if st.button("تسجيل الدخول",key="login_btn"):
                if username==ADMIN_USERNAME and password==ADMIN_PASSWORD:
                    ...
                    if ok: st.session_state.logged_in=True; st.session_state.office=result; st.session_state.update_locked=False; st.session_state.update_notice="idle"; st.rerun()
        ...
        with tab2:
            ...
            new_office=st.text_input("اسم المكتب",key="reg_office",placeholder="اسم المكتب"); new_email=st.text_input("الإيميل",key="reg_email",placeholder="example@email.com"); new_pass=st.text_input("كلمة المرور",type="password",key="reg_pass",placeholder="كلمة المرور"); new_pass2=st.text_input("تأكيد كلمة المرور",type="password",key="reg_pass2",placeholder="أعد كتابة كلمة المرور")
...
source_options=["📂 رفع ملف Excel","🔗 ربط Google Sheets"]; source=st.radio("",source_options,horizontal=True,label_visibility="collapsed",key="source_mode")
...
def render_processing():
    ...
    if st.session_state.update_locked and not st.session_state.active_job_id:
        ...
        if st.button("▶ تحديث حالات الطلاب",key="start_main"):
            ...
            if running:
                ...
                st.session_state.update_notice="waiting"
                ...
                # Keep the initial status visible on this run. The fragment will
                # replace it on the next poll once processing actually starts.
    ...
    if job:
        ...
        notice=st.session_state.get("update_notice","idle")
        if status in {"processing","done","failed"}:
            st.session_state.update_notice="started"
            notice="started"

        if status=="pending":
            st.info("⏳ سيبدأ التحديث خلال ثواني…")
        elif status in {"processing","done"} and notice=="started":
        ...
        if rows:
            ...
            for row_number,r in enumerate(reversed(shown),1):
                name=str(r.get("student_name") or "").strip(); status_text=str(r.get("status") or "").strip() or "لم يتم الفحص بعد"
        ...
        if status=="done":
            st.session_state.pending_file_bytes=None; st.session_state.pending_filename=""
