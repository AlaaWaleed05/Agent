...
def upload_to_drive(file_bytes, filename, office):
    """يرفع الملف ويرجع Google Drive file_id."""
    try:
        from googleapiclient.discovery import build
        ...
        metadata = {"name": f"{office}_{now}_{filename}", "parents": [DRIVE_FOLDER_ID]}
...
JOB_HEADERS = [
    "job_id", "اسم المكتب", "source_type", "source_link", "drive_file_id",
    "status", "total", "processed", "success", "failed", "current_student",
...
def get_update_jobs_sheet():
    client = get_gspread_client()
    ss = client.open_by_key(SHEET_ID)
    try:
        return ss.worksheet("update_jobs")
    except Exception:
        ws = ss.add_worksheet("update_jobs", 2000, len(JOB_HEADERS))
...
def get_worker_sheet():
    ...
    ss = client.open_by_key(SHEET_ID)
    try:
        return ss.worksheet("worker_status")
    except Exception:
        ws = ss.add_worksheet("worker_status", 10, 3)
        ws.append_row(["worker_id", "last_seen", "status"])
        return ws

def worker_is_online(max_age_seconds=60):
    try:
        ws = get_worker_sheet()
        rows = ws.get_all_records()
        if not rows:
            return False
        latest = rows[0].get("last_seen", "")
        if not latest:
            return False
        last = datetime.strptime(str(latest), "%Y-%m-%d %H:%M:%S")
        return (datetime.now() - last).total_seconds() <= max_age_seconds
    except Exception:
        return False

def create_update_job(office, source_type, total, source_link="", drive_file_id=""):
    ws = get_update_jobs_sheet()
    job_id = str(uuid.uuid4())
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ws.append_row([
        job_id, office, source_type, source_link, drive_file_id,
        "queued", total, 0, 0, 0, "", now, "", "", ""
    ])
    return job_id

def get_update_job(job_id):
    try:
        ws = get_update_jobs_sheet()
...
def render_job_progress(job_id):
    job = get_update_job(job_id)
    if not job:
        st.info("⏳ جاري تجهيز طلب التحديث...")
        return False
    ...
    current = str(job.get("current_student", "") or "")
    st.progress(min(processed / max(total, 1), 1.0))
    if status == "queued":
        st.info("⏳ الطلب في انتظار الـWorker على الجهاز...")
    elif status == "processing":
        st.markdown(f"**🔄 جاري تحديث:** {current or 'طالب'}")
        st.caption(f"تم تحديث {processed} من {total} · نجح {success} · فشل {failed}")
    elif status == "completed":
        st.success(f"🎉 اكتمل التحديث: {processed} طالب · نجح {success} · فشل {failed}")
        return True
    elif status == "failed":
        st.error(f"فشل التحديث: {job.get('error', '')}")
        return True
    return False
...
def find_excel_columns(ws):
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=1, max_row=5, values_only=True), start=1):
        ...
        if any("يميل" in v or "mail" in v.lower() for v in row_values):
            ...
            for i, cell in enumerate(row_values):
                lower = cell.lower()
                if any(k in cell for k in ["اسم", "الإسم", "الاسم"]) or "name" in lower:
                    cols["name"] = i
                elif any(k in cell for k in ["يميل", "بريد"]) or "mail" in lower:
                    cols["email"] = i
                elif any(k in cell for k in ["باسورد", "كلمة المرور", "password", "pass"]) or "pass" in lower:
                    cols["password"] = i
            break
    if header_row_num is None: raise Exception("مش لاقي هيدر الإكسيل!")
    if cols["email"] is None: raise Exception("مش لاقي عمود الإيميل!")
    if cols["password"] is None: raise Exception("مش لاقي عمود الباسورد!")
    if cols["status"] is None:
        ws.cell(row=header_row_num, column=header_len + 1, value="حالة الطلب الجديدة")
        cols["status"] = header_len
...
if file_bytes:
    ...
    if st.button("▶ تحديث حالات الطلاب", key="start_main"):
        log_to_sheet(office, "طلب تحديث", filename)
        ...
        valid_rows = [
            r for r in ws.iter_rows(min_row=header_row_num + 1, values_only=False)
            if r[cols["email"]].value and r[cols["password"]].value
        ]
        total = len(valid_rows)
        if total == 0:
            st.warning("مفيش طلاب صالحين للتحديث."); st.stop()

        # المصدر Google Sheet: الـWorker يقرأ من نفس الرابط.
        if sheet_id_source and source == "🔗 ربط Google Sheets":
            source_type = "gsheet"
            source_link = saved_link
            drive_file_id = ""
        else:
            ok, result = upload_to_drive(file_bytes, filename or "students.xlsx", office)
            if not ok:
                st.error(f"فشل تجهيز الملف: {result}"); st.stop()
            source_type = "drive"
            source_link = ""
            drive_file_id = result

        # لو الـWorker Online نرسل Job له.
        if worker_is_online():
            try:
                job_id = create_update_job(
                    office, source_type, total, source_link, drive_file_id
                )
                st.session_state.active_job_id = job_id
                st.success("🚀 تم إرسال التحديث للـWorker على الجهاز.")
            except Exception as e:
                st.error(f"فشل إنشاء طلب التحديث: {e}")

        # Fallback: لو الـWorker Offline، Streamlit ينفذ بالطريقة القديمة.
        else:
            st.warning("⚠️ الـWorker غير متصل الآن، سيتم استخدام التحديث الاحتياطي.")
            ...
            for idx, row in enumerate(valid_rows, start=1):
                ...
                name = str(row[cols["name"]].value or "") if cols["name"] is not None else ""
                status_placeholder.markdown(
                    f"**جاري تحديث:** {name or email}<br>"
                    f"<span style='color:#6b7280'>طالب {idx} من {total}</span>",
                    unsafe_allow_html=True
                )
            ...
            upload_to_drive(out.getvalue(), "students_updated.xlsx", office)
            ...
            save_results_to_sheet(office, st.session_state.last_results)
            log_to_sheet(office, "اكتمل المعالجة", filename)
            st.success(f"🎉 اكتمل التحديث · نجح {success} · فشل {failed}")

    active_job_id = st.session_state.get("active_job_id")
    if active_job_id:
        finished = render_job_progress(active_job_id)
        if not finished:
            time.sleep(2)
            st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
