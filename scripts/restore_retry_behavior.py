...
def patch_worker():
    ...
    old = '''        for index, student in enumerate(students, 1):
...
'''
    if old not in text:
        raise SystemExit("worker anchor 1")
...
'''
    if old not in text:
        raise SystemExit("worker anchor 2")
    text = text.replace(old, new, 1)

    old = '''        try:
            notify_office_status_changes(office, previous, processed)
'''
    new = '''        # Retry technical failures once after the first full pass.
        if retry_students:
...
'''
    if old not in text:
        raise SystemExit("worker anchor 3")
    text = text.replace(old, new, 1)
...
'''
    if old not in text:
        raise SystemExit("streamlit anchor 1")
    text = text.replace(old, new, 1)
...
'''
    if old not in text:
        raise SystemExit("streamlit anchor 2")
    text = text.replace(old, new, 1)

    old = '''        client.table("jobs").update({"status": "done", "finished_at": now_iso(), "error": None}).eq("id", job_id["id"]).execute()
'''
    new = '''        # Retry technical failures once after the first full pass.
        for index, student, name in retry_students:
            retry_status = TECH_FAILURE_STATUS
...
'''
    if old not in text:
        raise SystemExit("streamlit anchor 3")
    text = text.replace(old, new, 1)
    p.write_text(text, encoding="utf-8")
