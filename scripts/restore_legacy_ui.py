...
# Legacy visual system: fonts, spacing, cards, hero, inputs, tabs, uploader and buttons.
css = r'''<style>
...
if count != 1:
    raise RuntimeError('ui_css_block_not_found')

old_topbar = "st.markdown(\"<div class='topbar'><div class='brand-title'>Aivora</div><div class='brand-sub'>Your Smarter Support for Every Student's Application</div></div>\", unsafe_allow_html=True)"
new_topbar = '''st.markdown("""
<div class="topbar">
    <div class="brand">
...
# Restore greeting/hero without changing source/job logic.
anchor = 'office_id = office["id"]\n\n' + new_topbar
hero = '''office_id = office["id"]
...
if anchor not in text:
    raise RuntimeError('main_header_anchor_not_found')
text = text.replace(anchor, hero, 1)

# Old login copy/headings + placeholders; authentication logic remains current.
text = text.replace('''        with tab1:\n            username = st.text_input("اسم المكتب", key="login_user")''', '''        with tab1:
...
text = text.replace('''            password = st.text_input("كلمة المرور", type="password", key="login_pass")''', '''            password = st.text_input("كلمة المرور", type="password", key="login_pass", placeholder="اكتب كلمة المرور")''', 1)
text = text.replace('''        with tab2:\n            new_office = st.text_input("اسم المكتب", key="reg_office")''', '''        with tab2:
...
text = text.replace('''            new_email = st.text_input("الإيميل", key="reg_email")''', '''            new_email = st.text_input("الإيميل", key="reg_email", placeholder="example@email.com")''', 1)
text = text.replace('''            new_pass = st.text_input("كلمة المرور", type="password", key="reg_pass")''', '''            new_pass = st.text_input("كلمة المرور", type="password", key="reg_pass", placeholder="كلمة المرور")''', 1)
text = text.replace('''            new_pass2 = st.text_input("تأكيد كلمة المرور", type="password", key="reg_pass2")''', '''            new_pass2 = st.text_input("تأكيد كلمة المرور", type="password", key="reg_pass2", placeholder="أعد كتابة كلمة المرور")''', 1)

# Old admin hero/title, preserving current Supabase actions.
text = text.replace('''if st.session_state.is_admin:\n    st.markdown('<div class="card"><div class="section-title">الإدارة</div></div>', unsafe_allow_html=True)''', '''if st.session_state.is_admin:
    ...
    st.markdown(f"<div class='section-title'>الحسابات المعلقة <span style='color:#2563eb'>({len(pending)})</span></div>", unsafe_allow_html=True)''', 1)

# Old source/search copy; keep the single current uploader and all current logic.
text = text.replace('''<div class="section-title">مصدر بيانات الطلاب</div><div class="section-sub">اختاري المصدر الذي يحتوي على بيانات الطلاب.</div>''', '''<div class="section-title">مصدر بيانات الطلاب</div><div class="section-sub">اختر الطريقة التي يحتوي بها ملف الطلاب.</div>''', 1)
text = text.replace('''st.markdown('<div class="section-title">البحث عن طالب</div>', unsafe_allow_html=True)''', '''st.markdown('<div class="section-title">البحث عن طالب</div><div class="section-sub">اكتب اسم الطالب لمعرفة آخر حالة محفوظة.</div>', unsafe_allow_html=True)''', 1)

path.write_text(text, encoding='utf-8')
print('legacy UI restored')
